...
public class Miner extends NPCMob {
    ...
    public Miner(Location loc) {
        super("Miner",
                ...
                new MinerZombie(loc));
        // applying the cosmetics to the Miner npc
    ...
    private static class MinerZombie extends EntityZombie {
        public MinerZombie(Location loc) {
            ...
            setSilent(true);
            this.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).setValue((getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).getValue()*2)); // doubles zombie's speed
            this.getAttributeInstance(GenericAttributes.ATTACK_KNOCKBACK).setValue(69); //Haha funny knockback number
            this.getAttributeInstance(GenericAttributes.ARMOR).setValue(0); // resets the armor

            this.setPositionRotation(loc.getX(), loc.getY(), loc.getZ(), loc.getYaw(), loc.getPitch());

        }

        @Override // overriding this method will allow the zombie to not get on fire in day
